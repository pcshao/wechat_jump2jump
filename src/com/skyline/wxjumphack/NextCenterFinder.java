...
public class NextCenterFinder {
    ...
    public int[] find(BufferedImage image, int[] myPos) {
        ...
        System.out.println("最小RGB："+minR + ", " + minG + ", " + minB);
        System.out.println("最大RGB："+maxR + ", " + maxG + ", " + maxB);
        ...
        for (int j = height / 4; j < myPos[1]; j++) {
            for (int i = 0; i < width; i++) {
                ...
                int b = (pixel & 0xff);
                //找到rgb为以下表达式的存为ret[0]横坐标 ret[1]纵坐标
